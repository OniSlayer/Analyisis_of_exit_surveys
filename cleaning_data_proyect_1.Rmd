...
# Cease Date (Categorical, comes as text)
# Dete Start Date (Comes as text)
...
```{python}
# Looking for null values and column names and types
...
```{python}
# A deeper look into the null values

# calculating the percent of na values on each attribute
(dete_survey.isna().sum()/(dete_survey.shape[0])*100).sort_values(ascending = False)
```
